...
from GCN_model import GCN, create_graph_list_V
...
def train_valid_loop_ADAM(lr, dr, hidden_classes, trial_subdir, plot_subdir, task_name, batch_size, optimizer_name, trial):
    ...
    # Prepare to collect all predictions and trues for all subjects
    y_pred_all, y_true_all = [], []  # Collect all predictions and trues for all subjects

    # O2 Leave-One-Out: Outer loop for test subject
    val_optuna = []
    for test_idx in range(n):
        ...
        for val_idx in range(n):
            ...
            # Use the optimizer_name sampled by Optuna
...
def objective_ADAM(trial):
    ...
    hidden_classes = trial.suggest_int('hidden_classes', 16, 256)
...
taska_name = args.task_name
task_name = taska_name.replace("Task", "Task_").replace("_A", "")

global BASE_DIR

BASE_DIR = f"/rds/user/sss77/hpc-work/New/Graph/GCN_V/{task_name}"
task_dir_V = os.path.join("/rds/user/sss77/hpc-work/New/Graph/Graph_Data/Modify_Data", task_name)
...
print(f"[INFO] Task Name (V)   : {task_name}")
