...
def build_lf_model(audio_dim, video_dim, lstm_units, dropout_rate):
    class LateFusionLSTM(nn.Module):
        def __init__(self, audio_dim, video_dim, lstm_units, dropout_rate):
            super().__init__()
            self.audio_lstm = nn.LSTM(audio_dim, lstm_units, batch_first=True)
            self.video_lstm = nn.LSTM(video_dim, lstm_units, batch_first=True)
        ...
        def forward(self, audio, video):
            ...
            out = self.fc(out)
            return out
    return LateFusionLSTM(audio_dim, video_dim, lstm_units, dropout_rate)
...
def objective(trial, task_name, audio_data, video_data, labels, plot_dir, model_dir):
    ...
    for test_index in range(n_subjects):
        ...
        for val_index in train_val_ids:
            ...
            model = build_lf_model(X_train_audio_tensor.shape[2], X_train_video_tensor.shape[2], lstm_units, dropout_rate).to(device)
        ...
        if best_mae_val < float('inf') and best_weights is not None:
            model = build_lf_model(X_test_audio.shape[2], X_test_video.shape[2], lstm_units, dropout_rate).to(device)
...
model_dir = f'/rds/user/sss77/hpc-work/New/LSTM/LF/task_{task_name}'
