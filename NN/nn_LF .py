...
class LateFusionNN(nn.Module):
    def __init__(self, audio_dim, video_dim, hidden_dims, dropout):
        ...
        def make_mlp(input_dim, hidden_dims):
            ...
            for h in hidden_dims:
                ...
                input_dim = h
            layers.append(nn.Linear(input_dim, 1))  # Scalar output per branch
            return nn.Sequential(*layers)

        self.audio_mlp = make_mlp(audio_dim, hidden_dims)
        self.video_mlp = make_mlp(video_dim, hidden_dims)
...
def combined_ccc_mae_loss(y_true, y_pred, alpha=0.5):
    ccc_loss = diff_ccc_loss(y_true, y_pred)
    mae_loss = torch.nn.functional.l1_loss(y_true, y_pred)
    return alpha * ccc_loss + (1 - alpha) * mae_loss

def diff_ccc_loss(y_true, y_pred):
...
def objective(trial):
    ...
    for test_id in range(n_subjects):
        ...
        for val_id in train_val_ids:
            ...
            model = LateFusionNN(audio_data.shape[1], video_data.shape[1], hidden_dims, dropout).to(device)
            ...
            for epoch in range(epochs):
                ...
                #loss = diff_ccc_loss(output, y_train_scaled)
                ...
                model.eval()
                with torch.no_grad():
                    pred_val_scaled = model(X_val_audio_scaled, X_val_video_scaled).cpu().numpy()
                    pred_val = scaler_y.inverse_transform(pred_val_scaled)
                    mae_val = mean_absolute_error(y_val_tensor.cpu().numpy(), pred_val)
        ...
        final_model = LateFusionNN(audio_data.shape[1], video_data.shape[1], hidden_dims, dropout).to(device)
        ...
        final_model.eval()
        with torch.no_grad():
            pred_test_scaled = final_model(X_test_audio_scaled, X_test_video_scaled).cpu().numpy()
...
output_path = os.path.join(output_dir, "LF_completed.txt")
