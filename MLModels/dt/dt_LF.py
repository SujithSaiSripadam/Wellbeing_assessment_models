...
print(f"\nRunning DT + Optuna for task: {task_name}")
...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/late_fusion/dt/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/late_fusion/dt/plots_{task_name}/'
...
def objective(trial):
    ...
    # Hyperparams
    max_depth_audio = trial.suggest_int('max_depth_audio', 2, 20)
    max_depth_video = trial.suggest_int('max_depth_video', 2, 20)
    ...
    for test_id in range(n_subjects):
        ...
        for val_id in train_val_ids:
            ...
            audio_model = DecisionTreeRegressor(max_depth=max_depth_audio,
                                                min_samples_split=min_samples_split,
                                                min_samples_leaf=min_samples_leaf)
            video_model = DecisionTreeRegressor(max_depth=max_depth_video,
                                                min_samples_split=min_samples_split,
                                                min_samples_leaf=min_samples_leaf)
    ...
    plt.title(f"Late Fusion DT - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
...
# Save summary
with open(os.path.join(model_dir, f'results_dt_{task_name}.txt'), 'a') as f:
    f.write(f"\nTask: {task_name}\n")
