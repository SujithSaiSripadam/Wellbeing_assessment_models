...
print(f"\nRunning DT + Optuna (Early Fusion) for task: {task_name}")
...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/dt/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/dt/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Early Fusion DT - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
