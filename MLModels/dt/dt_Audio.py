...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/audio/dt/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/audio/dt/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Audio dt - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
...
# Save results
results_path = os.path.join(model_dir, f'results_dt_{task_name}.txt')
