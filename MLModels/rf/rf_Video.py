...
print(f"\nRunning Random Forest + Optuna (Video) for task: {task_name}")
...
# Save Directories
# ---------------------------
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/video/rf/'
...
def objective(trial):
    ...
    for test_id in range(n_subjects):
        test_x = video_data[test_id:test_id+1]
        ...
        for val_id in train_val_ids:
            ...
            X_train = video_data[train_ids]
            y_train = labels[train_ids]
            X_val = video_data[val_id:val_id+1]
    ...
    plt.title(f"Video RF - {task_name}")
    plt.xlabel("Fold")
    plt.ylabel("MAE")
    plt.grid(True)
