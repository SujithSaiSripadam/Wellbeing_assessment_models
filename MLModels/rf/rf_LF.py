...
print(f"\nRunning Random Forest + Optuna (Late Fusion) for task: {task_name}")
...
# Save Directories
# ---------------------------
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/late_fusion/rf/'
...
def objective(trial):
    ...
    # Shared hyperparameters for both audio and video regressors
    ...
    for test_id in range(n_subjects):
        test_audio = audio_data[test_id:test_id+1]
        test_video = video_data[test_id:test_id+1]
        test_label = labels[test_id:test_id+1]
        ...
        for val_id in train_val_ids:
            ...
            # Scaling
            scaler_a = StandardScaler().fit(X_train_audio)
            scaler_v = StandardScaler().fit(X_train_video)
            scaler_y = StandardScaler().fit(y_train.reshape(-1, 1))

            X_train_a_scaled = scaler_a.transform(X_train_audio)
            X_train_v_scaled = scaler_v.transform(X_train_video)
            y_train_scaled = scaler_y.transform(y_train.reshape(-1, 1)).ravel()

            X_val_a_scaled = scaler_a.transform(X_val_audio)
            X_val_v_scaled = scaler_v.transform(X_val_video)

            # Train models
            model_audio = RandomForestRegressor(
            ...
            model_video = RandomForestRegressor(
                n_estimators=n_estimators,
                max_depth=max_depth,
                min_samples_split=min_samples_split,
                min_samples_leaf=min_samples_leaf,
            ...
            model_audio.fit(X_train_a_scaled, y_train_scaled)
            model_video.fit(X_train_v_scaled, y_train_scaled)

            pred_val_audio = model_audio.predict(X_val_a_scaled)
            pred_val_video = model_video.predict(X_val_v_scaled)

            pred_val_avg = (pred_val_audio + pred_val_video) / 2.0
            pred_val_unscaled = scaler_y.inverse_transform(pred_val_avg.reshape(-1, 1)).ravel()
            val_mae = mean_absolute_error(y_val, pred_val_unscaled)

            if val_mae < best_val_mae:
                best_val_mae = val_mae
                best_audio_model = model_audio
                best_video_model = model_video
                best_scalers = (scaler_a, scaler_v, scaler_y)

        val_maes.append(best_val_mae)
        
        joblib.dump(model_audio, f'{plot_dir}/models/best_audio_model_{trial.number}.pkl')
        joblib.dump(model_video, f'{plot_dir}/models/best_video_model_{trial.number}.pkl')

        # Test prediction
        scaler_a, scaler_v, scaler_y = best_scalers
        X_test_a_scaled = scaler_a.transform(test_audio)
        X_test_v_scaled = scaler_v.transform(test_video)

        pred_a = best_audio_model.predict(X_test_a_scaled)
        pred_v = best_video_model.predict(X_test_v_scaled)

        pred_avg = (pred_a + pred_v) / 2.0
        final_pred = scaler_y.inverse_transform(pred_avg.reshape(-1, 1)).ravel()

        all_preds.append(final_pred[0])
        all_true.append(test_label[0])
    ...
    plt.title(f"Late Fusion RF - {task_name}")
    plt.xlabel("Fold")
    plt.ylabel("MAE")
    plt.grid(True)
...
print(f"\nLate Fusion Results for {task_name}:")
...
# Save results
# ---------------------------
results_path = os.path.join(model_dir, f'results_rf_latefusion_{task_name}.txt')
