...
import pandas as pd
# Get task name from the command line argument
...
# Use video features for training
X_combined = video_data

# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/video/svr-rbf/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/video/svr-rbf/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Video SVR-rbf - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
...
# Save results
results_path = os.path.join(model_dir, f'results_svr-rbf_{task_name}.txt')
