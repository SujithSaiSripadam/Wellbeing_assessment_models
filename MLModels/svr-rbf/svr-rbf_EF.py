...
print(f"\nRunning SVR + Optuna (Early Fusion) for task: {task_name}")
...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/svr-rbf/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/svr-rbf/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Early Fusion SVR - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
