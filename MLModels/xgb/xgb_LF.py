...
print(f"\nRunning XGBoost + Optuna for task: {task_name}")
...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/late_fusion/xgb/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/late_fusion/xgb/plots_{task_name}/'
...
def objective(trial):
    ...
    # Hyperparameters
    params_audio = {
        'n_estimators': trial.suggest_int('n_estimators_audio', 50, 300),
        'max_depth': trial.suggest_int('max_depth_audio', 2, 10),
        'learning_rate': trial.suggest_float('learning_rate_audio', 0.01, 0.3),
        'subsample': trial.suggest_float('subsample_audio', 0.5, 1.0),
        'colsample_bytree': trial.suggest_float('colsample_bytree_audio', 0.5, 1.0),
        'random_state': 42
    }

    params_video = {
        'n_estimators': trial.suggest_int('n_estimators_video', 50, 300),
        'max_depth': trial.suggest_int('max_depth_video', 2, 10),
        'learning_rate': trial.suggest_float('learning_rate_video', 0.01, 0.3),
        'subsample': trial.suggest_float('subsample_video', 0.5, 1.0),
        'colsample_bytree': trial.suggest_float('colsample_bytree_video', 0.5, 1.0),
        'random_state': 42
    }
    ...
    for test_id in range(n_subjects):
        ...
        for val_id in train_val_ids:
            ...
            audio_model = XGBRegressor(**params_audio)
            video_model = XGBRegressor(**params_video)
            ...
            if val_mae < best_val_mae_fold:
                ...
                best_video_model = video_model
                best_scalers = (scaler_audio, scaler_video, scaler_y)

        val_maes.append(best_val_mae_fold)
        fold_models.append((best_audio_model, best_video_model, best_scalers))
        ...
        y_pred_audio_test = scaler_y.inverse_transform(best_audio_model.predict(X_test_audio_scaled).reshape(-1, 1)).ravel()
        y_pred_video_test = scaler_y.inverse_transform(best_video_model.predict(X_test_video_scaled).reshape(-1, 1)).ravel()
    ...
    plt.title(f"Late Fusion XGBoost - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.grid(True)
    plt.legend()
