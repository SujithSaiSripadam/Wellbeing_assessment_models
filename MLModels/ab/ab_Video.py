...
labels = np.load('/rds/user/sss77/hpc-work/New/MLModels/Data/values_for3,5.npy') if task_name in ['3', '5'] else np.load('/rds/user/sss77/hpc-work/New/MLModels/Data/labels.npy')
labels = labels.astype(float)
n_subjects = len(labels)
X_combined = video_data

model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/video/ab/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/video/ab/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Video ab - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
...
with open(os.path.join(model_dir, f'results_ab_{task_name}.txt'), 'a') as f:
    f.write(f"\nTask: {task_name}\n")
