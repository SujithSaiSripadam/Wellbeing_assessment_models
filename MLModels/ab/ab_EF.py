...
print(f"\nRunning AdaBoost + Optuna (Early Fusion) for task: {task_name}")
...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/ab/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/early_fusion/ab/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Early Fusion AdaBoost - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
