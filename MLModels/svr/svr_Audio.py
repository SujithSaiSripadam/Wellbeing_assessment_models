...
# Save directories
model_dir = f'/rds/user/sss77/hpc-work/New/MLModels/audio/svr/'
plot_dir = f'/rds/user/sss77/hpc-work/New/MLModels/audio/svr/plots_{task_name}/'
...
def objective(trial):
    ...
    plt.title(f"Audio SVR - {task_name}")
    plt.xlabel('Fold')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
...
# Save results
results_path = os.path.join(model_dir, f'results_svr_{task_name}.txt')
